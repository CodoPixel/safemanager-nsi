...
function frame(min, n, max, strict = true) {
    if (strict) {
        return min < n && max > n;
    }
    else {
 ...
 */
function findNegativePatterns(str) {
    let score = 0;
...
function calcScoreOf(result) {
    ...
    if (result.usedWithConfig.maj)
        N += majAlpha.length;
    if (result.usedWithConfig.numbers)
        N += numbersAlpha.length;
    if (result.usedWithConfig.specialCharacters)
        N += specialCharactersAlpha.length;
    const strength = N ** (L - duplicationInfo.duplicatedChars.length - negativeScore);
    if (frame(1 ** 1, strength, 70 ** 8, false))
        return L > 6 && dupliScore < 2 ? 1 : 0;
    if (frame(70 ** 8 + 1, strength, 90 ** 11, false))
        return L > 10 && dupliScore < 3 ? 3 : 2;
    if (frame(90 ** 11 + 1, strength, 36 ** 15, false))
        return L > 13 && dupliScore < 10 ? 5 : 4;
    if (frame(36 ** 15 + 1, strength, 90 ** 15, false))
        return L >= 15 && dupliScore < 20 ? 7 : 6;
    if (frame(90 ** 15 + 1, strength, 90 ** 19, false))
        return L >= 16 && dupliScore < 30 ? 9 : 8;
    return 10;
}
/**
 ...
 */
function guessConfigFromPlainPassword(plainPassword) {
